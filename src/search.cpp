...
void update_all_stats(const Position& pos,
    ...
    {
        update_quiet_histories(pos, ss, workerThread, bestMove, bonus * (ss->inCheck ?  128 : 881) / 1024);

        // Decrease stats for all non-best quiet moves
        for (Move move : quietsSearched)
            update_quiet_histories(pos, ss, workerThread, move, -malus * (ss->inCheck ?  128 : 1083) / 1024);
    }
    else
    {
