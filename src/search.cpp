...
void  update_quiet_histories(
   const Position& pos, Stack* ss, Search::Worker& workerThread, bool pv, Move move, int bonus);
  ...
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
        ...
        && (ttData.bound & (ttData.value >= beta ? BOUND_LOWER : BOUND_UPPER)))
    {
        ...
        if (ttData.move && ttData.value >= beta)
        {
            ...
            if (!ttCapture)
                update_quiet_histories(pos, ss, *this, PvNode, ttData.move, stat_bonus(depth));
...
moves_loop:  // When in check, search starts here
    ...
    MovePicker mp(pos, ttData.move, depth, PvNode, &thisThread->mainHistory, &thisThread->PvHistory, &thisThread->captureHistory,
    ...
    // we update the stats of searched moves.
        ...
        update_all_stats(pos, ss, *this, bestMove, prevSq, quietsSearched, capturesSearched, depth, PvNode);
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta) {
    ...
    MovePicker mp(pos, ttData.move, DEPTH_QS, PvNode, &thisThread->mainHistory, &thisThread->PvHistory, &thisThread->captureHistory,
                  contHist, &thisThread->pawnHistory);
...
void update_all_stats(const Position&      pos,
    ...
    {
        update_quiet_histories(pos, ss, workerThread, pv, bestMove, quietMoveBonus);

        // Decrease stats for all non-best quiet moves
        for (Move move : quietsSearched)
            update_quiet_histories(pos, ss, workerThread, pv, move, -quietMoveMalus);
    }
    else
    {
...
void update_quiet_histories(
  const Position& pos, Stack* ss, Search::Worker& workerThread, bool pv, Move move, int bonus) {
    ...
    if (pv)
        workerThread.PvHistory[us][move.from_to()] << bonus;
